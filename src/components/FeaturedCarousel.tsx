...
/* ── Single card (used in mobile carousel & desktop grid) ── */
const SalonCard = memo(({ salon, onClick }: { salon: Salon; onClick: () => void }) => {
  const navigate = useNavigate();
  return (
    <div
      className="relative aspect-[16/10] cursor-pointer rounded-2xl overflow-hidden"
      onClick={onClick}
    >
      <img
        src={salon.image}
...
const FeaturedCarousel = memo(({ salons }: FeaturedCarouselProps) => {
  ...
  const prev = useCallback(() => {
    setCurrent((c) => (c - 1 + salons.length) % salons.length);
  }, [salons.length]);
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="relative overflow-hidden rounded-2xl mx-5 md:hidden"
      ...
      >
        ...
        >
          {salons.map((salon) => (
            <div key={salon.id} className="w-full flex-shrink-0">
              <SalonCard salon={salon} onClick={() => navigate(`/salon/${salon.id}`)} />
            </div>
          ))}
        </div>

        {/* Dots */}
        <div className="absolute bottom-2 right-3 flex gap-0 items-center">
          {salons.map((_, i) => (
            <button
              key={i}
            ...
            >
              <span className={`block rounded-full ${
                i === current ? 'w-3.5 h-[5px] bg-primary-foreground' : 'w-[5px] h-[5px] bg-primary-foreground/40'
      ...
      <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-4 px-5">
        {salons.map((salon) => (
          <SalonCard key={salon.id} salon={salon} onClick={() => navigate(`/salon/${salon.id}`)} />
        ))}
      </div>
    </>
  );
